        ...
        <div class="col-12">
            <div class="card card-primary">
                ...
                <div class="row col-12 mt-2" dir="rtl">
                    ...
                    {{-- mobile --}}
                    <x-adminlte-input name="mobile" label="رقم الموبايل" value="{{ old('mobile', $employee->mobile) }}"
                        placeholder="أدخل موبايل الموظف...." fgroup-class="col-md-4" />

                    {{-- alt_mobile --}}
                    <x-adminlte-input name="alt_mobile" label="موبايل آخر"
                        value="{{ old('alt_mobile', $employee->alt_mobile) }}" placeholder="أدخل موبايل آخر إن وجد...."
                        fgroup-class="col-md-4" />

                    {{-- username  --}}
                    <x-adminlte-input name="username" label="أسم المستخدم"
                        value="{{ old('username', $employee->username) }}" placeholder="أدخل أسم المستخدم...."
                        fgroup-class="col-md-4" />
                    ...
                    <x-adminlte-select label="نوع الحساب" fgroup-class="col-md-4" name="type">
                        ...
                        <option @if (old('type', $employee->type) == 'employee') selected @endif value="employee">موظف</option>
                        <option @if (old('type', $employee->type) == 'manager') selected @endif value="manager">مدير</option>
                    </x-adminlte-select>
                    ...
                    <x-adminlte-select label="نوع الجنس" fgroup-class="col-md-4" name="gender">
                        ...
                        <option @if (old('gender', $employee->gender) == 'male') selected @endif value="male">ذكر</option>
                        <option @if (old('gender', $employee->gender) == 'female') selected @endif value="female">أنثى</option>
                    </x-adminlte-select>

                    {{-- leave_balance --}}
                    <x-adminlte-input name="leave_balance" label="رصيد الأجازات"
                        value="{{ old('leave_balance', $employee->leave_balance) }}" placeholder="أدخل رصيد الأجازات 21...."
                        fgroup-class="col-md-4" />
                </div>
                ...
                <div class="row col-12" dir="ltr">
                    ...
                    @endphp
                    <x-adminlte-input-date name="birth_date" fgroup-class="col-6"
                        value="{{ old('birth_date', $employee->birth_date) }}" label="تاريخ الميلاد" igroup-size="sm"
                    ...
                    @endphp
                    <x-adminlte-input-date name="start_work" fgroup-class="col-6"
                        value="{{ old('start_work', $employee->start_work) }}" label="تاريخ بداية العمل" igroup-size="sm"
                        :config="$configStartWork" placeholder="أختر تاريخ بداية العمل" class="form-control">
                        <x-slot name="appendSlot">
                            <div class="input-group-text bg-dark">
                                <i class="fas fa-calendar-day"></i>
                            </div>
                    ...
                    <x-adminlte-select2 label="الفرع" fgroup-class="col-4" class="js-example-rtl" name="branch_id">
                        ...
                        @if (!empty($other['branches']) && isset($other['branches']))
                            @foreach ($other['branches'] as $branch)
                                <option @if (old('branch_id', $employee->branch_id) == $branch->id) selected @endif value="{{ $branch->id }}">
                                    {{ $branch->name }}</option>
                            @endforeach
                        @else
                            لا توجد بيانات
                    ...
                    <x-adminlte-select2 label="الراحه الاسبوعية" fgroup-class="col-4" class="js-example-rtl"
                        ...
                        @if (!empty($other['weekly_rests']) && isset($other['weekly_rests']))
                            @foreach ($other['weekly_rests'] as $weekly)
                                <option @if (old('weekly_rest_id', $employee->weekly_rest_id) == $weekly->id) selected @endif value="{{ $weekly->id }}">
                                    {{ $weekly->name }}</option>
                            @endforeach
                        @else
                            لا توجد بيانات
                    ...
                    <x-adminlte-select2 label="الدرجه الوظيفية" fgroup-class="col-4" class="js-example-rtl"
                        ...
                        @if (!empty($other['job_grades']) && isset($other['job_grades']))
                            @foreach ($other['job_grades'] as $job)
                                <option @if (old('job_grade_id', $employee->job_grade_id) == $job->id) selected @endif value="{{ $job->id }}">
                                    {{ $job->name }}</option>
                            @endforeach
                        @else
                            لا توجد بيانات
